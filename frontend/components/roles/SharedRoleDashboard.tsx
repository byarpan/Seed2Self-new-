...
export default function SharedRoleDashboard({ user, role }: { user: any; role: string }) {
  ...
  const fetchData = async () => {
    try {
      const roleIdKey = role.toLowerCase() + "Id";
      const ownerId = user[roleIdKey] || user.id;
  ...
  const roleIdKey = role.toLowerCase() + "Id";
  const currentUserId = user[roleIdKey] || user.id;

  return (
    <div className="min-h-screen relative text-white">
      <Head>
        <title>{role} Dashboard | Seed2Shelf</title>
      </Head>


      <div className="relative z-10 max-w-7xl mx-auto px-6 py-12">
        <h1 className="text-4xl font-bold mb-8 text-green-400 drop-shadow-md">{role} Logistics Portal</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col gap-8">
            ...
            <div className="matte-glass p-8 rounded-3xl shadow-2xl border border-white/10">
              ...
              {orders.filter(o => o.sellerId === currentUserId).length === 0 ? (
                ...
                <div className="space-y-5 overflow-y-auto pr-2 custom-scrollbar max-h-[400px]">
                  {orders.filter(o => o.sellerId === currentUserId).map(order => (
            ...
            <div className="matte-glass p-8 rounded-3xl shadow-2xl border border-white/10">
              ...
              {orders.filter(o => o.buyerId === currentUserId).length === 0 ? (
                <p className="text-stone-400 italic text-sm">No active purchases.</p>
              ) : (
                <div className="space-y-5 overflow-y-auto pr-2 custom-scrollbar max-h-[400px]">
                  {orders.filter(o => o.buyerId === currentUserId).map(order => (
                    <div key={order.orderId} className="p-6 bg-white/5 border border-white/10 rounded-2xl flex flex-col hover:bg-white/10 transition-colors">
                      <p className="text-lg font-medium text-stone-100 mb-2">
