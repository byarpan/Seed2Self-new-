...
export default function ProcessorDashboard({ user }: { user: any }) {
  ...
  const fetchData = async () => {
    try {
      const ownerId = user.processorId || user.id;

      // 1. Fetch Processor Inventory from MongoDB
      ...
      console.error("Error fetching processor data:", err);
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col gap-8">
            ...
            <div className="matte-glass p-8 rounded-3xl shadow-2xl border border-white/10">
              ...
              {orders.filter(o => o.sellerId === (user.processorId || user.id)).length === 0 ? (
                ...
                <div className="space-y-5 overflow-y-auto pr-2 custom-scrollbar max-h-[400px]">
                  {orders.filter(o => o.sellerId === (user.processorId || user.id)).map(order => (
            ...
            <div className="matte-glass p-8 rounded-3xl shadow-2xl border border-white/10">
              ...
              {orders.filter(o => o.buyerId === (user.processorId || user.id)).length === 0 ? (
                <p className="text-stone-400 italic text-sm">No active purchases.</p>
              ) : (
                <div className="space-y-5 overflow-y-auto pr-2 custom-scrollbar max-h-[400px]">
                  {orders.filter(o => o.buyerId === (user.processorId || user.id)).map(order => (
                    <div key={order.orderId} className="p-6 bg-white/5 border border-white/10 rounded-2xl flex flex-col hover:bg-white/10 transition-colors">
                      <p className="text-lg font-medium text-stone-100 mb-2">
